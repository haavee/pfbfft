import math, random, numpy, itertools, rdvdif3, operator
...
sum_it = lambda *args: sum(list(args))

def add(*signals):
    return map(sum, zip(*signals))
    #return map(sum_it, zip(*signals))
    #return itertools.imap(sum, itertools.izip(*signals))

def mix(sig1, sig2):
    while True:
        yield next(sig1) * next(sig2)
...
def sine_embedded_in_noise(freq, SNR=0.001):
    return add(gaussion_noise(), sinewave(1.0/freq

def mk_sig(n, amps, ampn, period=16, phase=0):
    return mk_sig2(n, gaussian_noise(amplitude=ampn), sinewave(period, amplitude=amps, phase=phase))
